...
export function exportReactComponent(config: AsciiConfig): string {
  // Create config without width/height for export
  const { width, height, ...exportConfig } = config;

  return `"use client";

import { useRef, useEffect, useCallback } from 'react';

export default function AsciiAnimation({ className, style } = {}) {
  ...
  const config = {
    ...${JSON.stringify(exportConfig, null, 2).replace(/\n/g, "\n    ")},
...
\`;
  ...
  const render = useCallback((tMs) => {
    ...
    gl.uniform1f(uNoise.uNoiseStrength, config.noiseStrength);
    gl.uniform1f(uNoise.uNoiseScale, config.noiseScale);
    gl.uniform1f(uNoise.uSpeed, config.speed);
    gl.uniform3f(uNoise.uTint, config.tint[0], config.tint[1], config.tint[2]);
    gl.uniform1f(uNoise.uDistortAmp, config.distortAmp);
    gl.uniform1f(uNoise.uFrequency, config.frequency);
    gl.uniform1f(uNoise.uZRate, config.zRate);
    gl.uniform1f(uNoise.uBrightness, config.brightness);
    gl.uniform1f(uNoise.uContrast, config.contrast);
    gl.uniform1f(uNoise.uSeed1, config.seed1);
    gl.uniform1f(uNoise.uSeed2, config.seed2);
    gl.uniform1f(uNoise.uHue, config.hue);
    gl.uniform1f(uNoise.uSaturation, config.saturation);
    gl.uniform1f(uNoise.uGamma, config.gamma);
    gl.uniform1f(uNoise.uVignette, config.vignette);
    gl.uniform1f(uNoise.uVignetteSoftness, config.vignetteSoftness);
    gl.uniform1f(uNoise.uGlyphSharpness, config.glyphSharpness);
    gl.uniform3f(uNoise.uBg, config.bg[0], config.bg[1], config.bg[2]);
    ...
    gl.uniform1f(uAscii.uCell, config.cell);
    gl.uniform1i(uAscii.uBW, config.bw ? 1 : 0);
    gl.uniform1i(uAscii.uCharset, charsetMap[config.charsetPreset]);
    gl.uniform1f(uAscii.uBrightness, config.brightness);
    gl.uniform1f(uAscii.uContrast, config.contrast);
    gl.uniform3f(uAscii.uTint, config.tint[0], config.tint[1], config.tint[2]);
    gl.uniform1f(uAscii.uTime, t);
    gl.uniform1f(uAscii.uDistortAmp, config.distortAmp);
    gl.uniform1f(uAscii.uFrequency, config.frequency);
    gl.uniform1f(uAscii.uZRate, config.zRate);
    gl.uniform1f(uAscii.uSeed1, config.seed1);
    gl.uniform1f(uAscii.uSeed2, config.seed2);
    gl.drawArrays(gl.TRIANGLES, 0, 6);

    if (config.duration > 0 && t >= config.duration && config.loop) {
      startRef.current = tMs;
    }
