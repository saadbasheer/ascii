...
const charsetMap: Record<AsciiConfig["charsetPreset"], number> = {
  full: 0,
  minimal: 1,
  medium: 2,
  custom: 0,
  ...
  ({ config, className, style }, ref) => {
    ...
    const init = useCallback((gl: Gl, w: number, h: number) => {
      ...
      const uNoise = {
        ...
        uVignetteSoftness: gl.getUniformLocation(
          progNoise,
          "uVignetteSoftness"
        )!,
      ...
      const uAscii = {
        ...
        uSourceResolution: gl.getUniformLocation(
          progAscii,
          "uSourceResolution"
        )!,
    ...
    const render = useCallback(
      (tMs: number) => {
        ...
        if (!res || !canvas || !isRunning.current) return;
    ...
    }, [config.width, config.height, init, render]);
